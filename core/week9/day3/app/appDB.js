...
app.put('/catedrales/:id', (req, res) => {
    ...
    pool.query('SELECT * FROM catedrales WHERE IDCatedral = ' + id, (err, [catedral]) => {
        if(!catedral) return res.status(404).send("No existe esa catedral en nuestra base de datos")
        ...
        pool.query("UPDATE catedrales SET ? WHERE IDCatedral = ? ", [req.body, id], (err, rows) => {
            if(err) return res.status(500).send('Espera que solucionemos algo')

            res.json({...catedral, ...req.body});
        })
    })
})
